...
public class JdbcUserDao implements UserDao {
    ...
    @Override
    public List<UsernamesDTO> findAllExceptCurrent(int currentUserId) {
        List<UsernamesDTO> users = new ArrayList<>();
    ...
    public boolean create(String username, String password) {
        ...
        // implemented account creation when creating a new user
        String sqlInsertAccount = "INSERT INTO account (user_id, balance) VALUES (?, ?)";
        BigDecimal initialBalance = new BigDecimal("1000.00");
        try {
            jdbcTemplate.update(sqlInsertAccount, newUserId, initialBalance);
            return true;
        } catch (DataAccessException e) {
            jdbcTemplate.update("DELETE FROM tenmo_user WHERE user_id = ?", newUserId);
            return false;
        }
    }
